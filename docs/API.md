...
| `/api/lifecycle/daily` | GET | Cron | Daily onboarding emails. Currently the day-2 **diagnostic nudge** (parent added a child but ran no diagnostic). Requires `Authorization: Bearer $CRON_SECRET`; scheduled daily 09:00 UTC via `vercel.json`. Each lifecycle email is claimed in Mongo before sending (`lifecycle_emails_sent`), so it's sent at most once even across overlapping runs; respects `marketing_emails_opt_out`. Welcome + first-plan emails are transactional (fire from verification / schedule-approval), not here. 503 when `CRON_SECRET` or `BREVO_API_KEY` is unset. |
