...
/**
 * Parent-facing, print-beautiful certificate for a single mastered topic (F3) —
 * the downloadable evidence behind the "Certificates" section on the child
 * profile. Same trust primitive (SHA-256 over canonical facts) and print-to-PDF
 * pattern as the subject-level mastery certificate; dark on screen, clean on
 * paper. On-screen chrome is hidden when printing.
 */
  ...
  return (
    <div className="cert-root min-h-screen bg-void px-6 py-10 print:bg-white print:px-0 print:py-0">
      ...
      <article className="cert-paper mx-auto max-w-4xl rounded-3xl border-4 border-amber-400/30 bg-white/[0.03] p-12 text-center shadow-2xl print:rounded-none print:border-amber-700">
        ...
        <p className="mt-8 text-base text-fog-300">has mastered</p>
        <p className="mt-1 text-2xl font-semibold text-fog-50">
          {certificate.topicTitle}
        </p>
        <p className="mt-1 text-sm text-fog-400">{certificate.subjectLabel}</p>
